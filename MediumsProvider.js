...
const MediumsProvider = ({children, userId}) => {
  ...
  const createMedium = (newMediumName, newMediumLink) => {
    const realm = realmRef.current;
    ...
    realm.write(() => {
      ...
      realm.create(
        ...
        new Medium({name: newMediumName || 'New Medium', link: newMediumLink, partition: userId}),
  ...
  const setMediumStatus = (medium, status) => {
    // One advantage of centralizing the realm functionality in this provider is
    // that we can check to make sure a valid status was passed in here.
    if (
      ![
        Medium.STATUS_OPEN,
        Medium.STATUS_HIBERNATED,
        Medium.STATUS_ARCHIVED,
      ].includes(status)
    ) {
